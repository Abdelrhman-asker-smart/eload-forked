...
// api().fetchTeamData
export const fetchShipmentList = createAsyncThunk(
  "shipments/fetchShipmentList",
  async (token) => {
...
export const shipmentGetReducer = createSlice({
  name: "categories",
  initialState: {
    list: [],
    // tableContainer: [],
  ...
  extraReducers: {
    [fetchShipmentList.pending]: (state, action) => {
      // state.status = true;
    },
    [fetchShipmentList.fulfilled]: (state, action) => {
      state.status = false;
    ...
    [fetchShipmentList.rejected]: (state, action) => {
      state.status = true;
      state.status = "failed";
    },
  },
...
export default shipmentGetReducer.reducer;
