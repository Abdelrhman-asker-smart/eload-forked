...
export const partnerDriverGetReducer = createSlice({
  name: "partner",
  initialState: {
    list: [],
    // tableContainer: [],
  ...
  extraReducers: {
    [fetchPartDriverList.pending]: (state, action) => {
      // state.status = true;
    },
    [fetchPartDriverList.fulfilled]: (state, action) => {
      state.status = false;
    ...
    [fetchPartDriverList.rejected]: (state, action) => {
      state.status = true;
      state.status = "failed";
    },
  },
...
export default partnerDriverGetReducer.reducer;
