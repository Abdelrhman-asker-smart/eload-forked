  ...
  async (token) => {
    ...
    try {
      const response = await axios.get(
        "https://dev.eload.smart.sa/api/v1/promotions",
        {
...
export const PromotionGetReducer = createSlice({
  name: "promotion",
  initialState: {
    list: [],
    // tableContainer: [],
  ...
  extraReducers: {
    [fetchPromotionList.pending]: (state, action) => {
      // state.status = true;
    },
    [fetchPromotionList.fulfilled]: (state, action) => {
      state.status = false;
    ...
    [fetchPromotionList.rejected]: (state, action) => {
      state.status = true;
      state.status = "failed";
    },
  },
...
export default PromotionGetReducer.reducer;
