...
export const UOMGetReducer = createSlice({
  name: "categories",
  initialState: {
    list: [],
    // tableContainer: [],
  ...
  extraReducers: {
    [fetchUOMList.pending]: (state, action) => {
      // state.status = true;
    },
    [fetchUOMList.fulfilled]: (state, action) => {
      state.status = false;
    ...
    [fetchUOMList.rejected]: (state, action) => {
      state.status = true;
      state.status = "failed";
    },
  },
...
export default UOMGetReducer.reducer;
