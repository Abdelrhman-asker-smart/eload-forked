...
export const TruckGetReducer = createSlice({
  name: "truck",
  initialState: {
    list: [],
    // tableContainer: [],
  ...
  extraReducers: {
    [fetchTruckList.pending]: (state, action) => {
      // state.status = true;
    },
    [fetchTruckList.fulfilled]: (state, action) => {
      state.status = false;
    ...
    [fetchTruckList.rejected]: (state, action) => {
      state.status = true;
      state.status = "failed";
    },
  },
...
export default TruckGetReducer.reducer;
